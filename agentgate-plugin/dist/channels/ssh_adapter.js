...
export class SSHAdapter {
    ...
    async start() {
        ...
        return new Promise((resolve, reject) => {
            this.server = new SshServer({ hostKeys: [hostKey] }, (client) => this.handleClient(client));
            this.server.on('error', (err) => { console.error(`[SSH] ${err.message}`); if (!this.running)
                reject(err); });
    ...
    async send(envelope) {
        ...
        if (!session) {
            console.warn(`[SSH] No session for ${envelope.channel_user_id}`);
            return;
        }
        try {
            session.write(`\n[Agent] ${text}\n\n> `);
    ...
    handleClient(client) {
        ...
        client.on('authentication', (ctx) => {
            ...
            if (ctx.method === 'password') {
                ...
                if (users[username] && pwdCtx.password === users[username]) {
                    userId = username;
                    ctx.accept();
                    return;
                }
            ...
            client.on('session', (accept) => {
                ...
                session.on('shell', (accept) => {
                    ...
                    stream.on('data', (data) => {
                        ...
                        if (buffer.endsWith('\x7f') || buffer.endsWith('\b')) {
...
//# sourceMappingURL=ssh_adapter.js.map