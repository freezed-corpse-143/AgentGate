...
export class RegistryServer {
    ...
    onConnect(socket) {
        ...
        const conn = { socket, agent_id: null, port: 0 };
        ...
        socket.on('data', (chunk) => {
            ...
            for (const msg of msgs)
                this.handleMsg(conn, msg, () => { registered = true; });
        });
        socket.on('close', () => {
...
export class BridgeAgent {
    ...
    // 直连 peers
    peers = new Map();
    seenFromPeers = new Set();
    // 心跳
    heartbeatTimer = null;
    // 是否自举为注册中心
    isRegistryBootstrap = false;
    ...
    handleIncoming(socket) {
        ...
        socket.on('data', (chunk) => {
            ...
            for (const msg of msgs) {
                if (msg.type === 'register') {
                    ...
                    try {
                        socket.write(encode({ type: 'heartbeat_ack' }));
                    }
                    catch { }
                }
        ...
        socket.on('close', () => { if (peerInfo.agent_id)
            this.peers.delete(peerInfo.agent_id); });
        socket.on('error', () => { });
    }
    // ─── 注册中心连接与自举 ────────────────────────
    async tryConnectRegistry() {
        // 先尝试连接已有注册中心
    ...
    sendToRegistry(msg) {
        try {
            this.registrySocket?.write(encode(msg));
        }
        catch { }
    }
    // ─── 处理注册中心的消息 ─────────────────────────
    handleRegistryMsg(msg) {
    ...
    // ─── Peer 直连管理 ─────────────────────────────
    connectToPeer(info) {
        if (info.agent_id === this.agentId)
            return; // 不连自己
        if (this.peers.has(info.agent_id))
            return; // 已连接
        const peer = { agent_id: info.agent_id, host: info.host, port: info.port, socket: null, reconnectTimer: null };
        ...
        s.on('data', (chunk) => {
            ...
            for (const msg of msgs) {
                if (msg.type === 'heartbeat') {
                    try {
                        s.write(encode({ type: 'heartbeat_ack' }));
                    }
                    catch { }
                }
    ...
    handlePeerMessage(msg) {
        if (msg.envelope.message_id && this.seenFromPeers.has(msg.envelope.message_id))
            return;
        if (msg.envelope.message_id) {
            this.seenFromPeers.add(msg.envelope.message_id);
            if (this.seenFromPeers.size > SEEN_SET_MAX)
                this.seenFromPeers.clear();
        }
        this.bus.publish(msg.topic, msg.envelope);
    }
    /** 订阅本地 bus，将需要远程投递的消息转发给对应 peer */
    subscribeBus() {
        this.bus.subscribeWildcard('agent.*.inbound', (envelope) => {
            this.routeToPeer(envelope.agent_id, envelope);
        });
        this.bus.subscribeWildcard('agent.*.outbound', (envelope) => {
            this.routeToPeer(envelope.agent_id, envelope);
        });
        this.bus.subscribeWildcard('_system.*', (envelope, topic) => {
            this.broadcastToPeers(topic, envelope);
        });
    }
    routeToPeer(targetAgentId, envelope) {
        if (targetAgentId === this.agentId)
            return; // 本地消息，已通过 bus 直接处理
    ...
    broadcastToPeers(topic, envelope) {
    ...
    sendHeartbeats() {
        ...
        for (const [aid, p] of this.peers) {
            if (!p.socket)
                continue;
            try {
                p.socket.write(encode({ type: 'heartbeat' }));
            }
            catch { }
        }
    }
}
//# sourceMappingURL=peer_bridge.js.map