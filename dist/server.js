...
export async function startServer(config, opts) {
    ...
    if (!opts?.headless) {
        if (config.channels.rest?.enabled) {
            ...
            ra.onMessage((raw) => { gateway.receive(raw).catch(err => console.error(`[Gateway] Error: ${err}`)); });
            adapters.push(ra);
        }
        ...
        for (const tc of telegramConfigs) {
            if (!tc.enabled || !tc.token)
                continue;
            const ta = new TelegramAdapter({ token: tc.token, apiRoot: tc.apiRoot, allowFrom: tc.allowFrom });
            ta.onMessage((raw) => { gateway.receive(raw).catch(err => console.error(`[Gateway] Error: ${err}`)); });
            adapters.push(ta);
        ...
        if (config.channels.ssh?.enabled) {
            ...
            sa.onMessage((raw) => { gateway.receive(raw).catch(err => console.error(`[Gateway] Error: ${err}`)); });
            adapters.push(sa);
        }
        // 广播信道（始终启用）
        const ba = new BroadcastAdapter(bus);
        ba.onMessage((raw) => { gateway.receive(raw).catch(err => console.error(`[Gateway] Error: ${err}`)); });
        adapters.push(ba);
        // 订阅管理器
