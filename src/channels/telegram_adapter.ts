...
import { readFileSync, writeFileSync, mkdirSync, rmSync, statSync, realpathSync } from 'fs'
...
export class TelegramAdapter implements ChannelAdapter {
  ...
  constructor(options: TelegramAdapterOptions) {
    ...
    this.bot = new Bot(options.token, botConfig as any)
  ...
  private setupBot(): void {
    ...
    this.bot.on('message:text', async (ctx: Context) => {
      ...
      if (chatType === 'private') {
        ...
        // 群组: 默认只响应 @mention
        if (!this.isMentioned(ctx)) return
      }
