...
export function generateConversationId(channel: ChannelType, userId: string): string {
 ...
 */
export function createEnvelope(options: CreateEnvelopeOptions): Envelope {
...
const REQUIRED_FIELDS: (keyof Envelope)[] = [
  'message_id',
  'trace_id',
  'channel',
  'channel_user_id',
...
const VALID_CHANNELS: string[] = ['telegram', 'rest', 'websocket', 'ssh', 'tailscale', 'agentgate']
const VALID_DIRECTIONS: string[] = ['inbound', 'outbound']
 ...
 * 缺少必填字段或字段值非法时返回错误列表。
 */
export function validateEnvelope(env: Envelope): ValidationResult {
  const errors: string[] = []
 ...
 */
export function hasRouteLoop(env: Envelope): boolean {
 ...
 */
export function wasProcessedBy(env: Envelope, agentId: string): boolean {
