...
import type { AgentGateServer } from './server.js'

// ─── 全局状态 ──────────────────────────────────────────────────

let server: AgentGateServer | null = null
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.port) {
      if (config.channels.rest) config.channels.rest.port = opts.port
    }
    if (opts.config) process.env.AGENTGATE_CONFIG = opts.config
    if (opts.bridge) {
...
program
  ...
  .action(async (opts) => {
    ...
    } catch { registryStatus = 'check failed' }
