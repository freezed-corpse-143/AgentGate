...
export class HandshakeManager {
  ...
  private cache: PendingPairing[] | null = null

  constructor(store: BindingStore) {
    this.store = store
  }

  private load(): PendingPairing[] {
    if (this.cache) return this.cache
    try {
      const raw = readFileSync(PENDING_FILE, 'utf8')
      this.cache = JSON.parse(raw) as PendingPairing[]
    } catch (err) {
  ...
  private save(): void {
    ...
    try {
      ...
      try { writeFileSync(PENDING_FILE, JSON.stringify(this.cache, null, 2)) } catch {}
    }
  }
  ...
  getPending(channel: ChannelType, channelUserId: string): PendingPairing | undefined {
    return this.load().find(
      p => p.channel_type === channel && p.channel_user_id === channelUserId,
    )
  }
  ...
  createPairing(channel: ChannelType, channelUserId: string, chatId: string): string {
    ...
    const existing = this.cache!.find(
      p => p.channel_type === channel && p.channel_user_id === channelUserId && p.expires_at > Date.now(),
    )
